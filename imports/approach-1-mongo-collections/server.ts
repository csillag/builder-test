...
Meteor.methods({
    d1SetCertified: (id: string, value: boolean) => CompanyCollection.update(id, { $set: { certified: value } }),
    d1HideCompany: (id: string) => CompanyCollection.update(id, { $set: { hidden: true } }),
    d1ShowAllCompanies: () => CompanyCollection.update({ hidden: true }, { $set: { hidden: false } }),
});
