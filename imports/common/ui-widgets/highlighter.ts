 ...
 */
export const highlightSearchResult = (value: string, searchText: string | undefined): string => {
    const match = findMatch(value, [searchText]);
    if (match) {
        return value.replace(RegExp(match.searchText, 'i'), (text) => '<span class="search-match">' + text + '</span>');
    } else {
        return value;
    }
};
